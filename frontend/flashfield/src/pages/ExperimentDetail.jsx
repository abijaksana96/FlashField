...
// Komponen kecil untuk menampilkan info dengan ikon
...
function ExperimentDetail() {
    ...
    useEffect(() => {
        const fetchExperimentDetails = async () => {
            setLoading(true);
            try {
                // Panggil endpoint BARU untuk mendapatkan semua detail field
                const response = await apiClient.get(`/experiments/${id}/fields`);
    ...
    if (!experimentDetails) return <ErrorMessage message="Data eksperimen tidak ditemukan." />;

    return (
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            {/* --- Header Detail Eksperimen --- */}
            <div className="border-b border-navy/50 pb-6 mb-8">
                <h1 className="text-4xl md:text-5xl font-extrabold text-lightest-slate mb-3">{experimentDetails.title}</h1>
                {/* Info Pills untuk Deadline & Lokasi */}
                <div className="flex flex-wrap gap-4">
                    <InfoPill 
                        ...
                        text={`Deadline: ${formatDate(experimentDetails.deadline)}`}
                    />
                    <InfoPill 
                        ...
                        text={experimentDetails.require_location ? "Memerlukan Lokasi" : "Tidak Memerlukan Lokasi"}
                    />
                </div>
            </div>
            ...
            <div className="bg-light-navy p-6 rounded-lg">
                 ...
                 {experimentDetails.input_fields && experimentDetails.input_fields.length > 0 ? (
                    <ul className="list-disc list-inside space-y-2 text-slate">
                        {experimentDetails.input_fields.map(field => (
                            <li key={field.name}>
